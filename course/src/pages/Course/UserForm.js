...
const UserForm = ({ setRegisterResult, courseId, setError }) => {
  ...
  const registerMinDate = React.useMemo(() => {
    if (
      new Date(moment().add(7, "day").format("YYYY-MM-DD")) <
      new Date("2020-09-26")
    ) {
      return new Date("2020-09-26");
    } else {
      return new Date(moment().add(7, "day").format("YYYY-MM-DD"));
    }
  }, []);
